...
history_object = model.fit(X_train_shuffled,Y_train_shuffled,validation_split=0.2,shuffle=True,nb_epoch=5)
...
#import model
model = load_model('model_nvidia3.h5')
#read in images
...
d4 = deque(maxlen=50)
d5 = deque(maxlen=75)
for filename in np.sort(os.listdir(input_dir)).tolist():
    if filename.endswith('.jpg'):
        ...
        #use a rolling average of heat maps over the past f frames
        d1.append(heatmap.astype(float).copy()+2.0)
        f1 = np.average(d1,axis=0)
        d2.append(heatmap.astype(float).copy()+2.0)
        f2 = np.average(d2,axis=0)
        d3.append(heatmap.astype(float).copy()+2.0)
        f3 = np.average(d3,axis=0)
        """d4.append(heatmap.astype(float).copy()+2.0)
        f4 = np.average(d4,axis=0)
        d5.append(heatmap.astype(float).copy()+2.0)
        f5 = np.average(d5,axis=0)
        if len(d4)==50:
            f4 = np.average(list(itertools.islice(d4, 25, 50)),axis=0)
        if len(d5)==75:
            f5 = np.average(list(itertools.islice(d5, 50, 75)),axis=0)
        final_heatmap = np.ceil(np.average(np.stack((f1,f2,f3,f4,f5)),axis=0))"""
        final_heatmap = np.ceil(np.average(np.stack((f1,f2,f3)),axis=0))
        #apply threshold
        thresh_heatmap = apply_threshold(final_heatmap, threshold=6)
